...
This was solved by replacing the parameter in line 323 'taxa_are_rows = F' to taxa_are_rows = T'
...
```{r}
# Separate accession numbers into corresponding sample
...
total = median(sample_sums(DSH08))
standf = function(x, t=total) round(t *(x/ sum(x)))
DSH08 = transform_sample_counts(DSH08, standf)

```

## producing an ordered plot
```{r}
...
Class_abun <- pd %>%
  ...
  arrange(Abundance)
Class_levels <- Class_abun$Class
pd0 <- pd %>%
  mutate(Class = factor(Class, Class_levels))
ggplot(pd0, aes(x = depth, y = Abundance, fill = Class )) + geom_bar(stat="identity") + ggtitle("DSH08 1100m") + scale_x_discrete(name = "Depth (cm) ", limits = c(0, 5, 10, 15, 20))
