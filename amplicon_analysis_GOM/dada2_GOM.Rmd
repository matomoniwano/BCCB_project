...
sample_info_tab <- read.table("sample_metadata.txt", header = T, row.names = NULL, check.names = F, sep = "\t")
...
otu <- otu_table(count_tab, taxa_are_rows = F)
TAX <- tax_table(tax_tab)
samples <- sample_data(sample_info_tab)

row.names(otu) <- paste0("ASV", 1)
row.names(TAX) <- TAX[[ASVs]]
row.names(samples) <- samples[[run_id]]

otu <- otu %>% select (-ASVs)
TAX <- TAX %>% select (-ASVs)
samples <- samples %>% select (-run_id)
